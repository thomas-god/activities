...
<script lang="ts">
	...
	$effect(() => {
		...
		if (isNone(syncHoveredBin) && _tooltip.source === 'external') {
		...
		const row = rows.find((r) => r.__ts === dayjs(time).unix());
		const total = row === undefined ? 0 : d3.sum(groups, (g) => row[g] ?? 0);
		const value = points.find((v) => v.time === time && v.group === group)?.value;
		const yPos = yAxis(total);
		...
		d3.select(gPaths).call((sel) =>
			sel
				...
				.on('mousemove', function (event: MouseEvent, s: d3.Series<StackRow, string>) {
					...
					if (bin !== undefined) {
						syncHoveredBin = some({ bin, group: s.key });
					}
