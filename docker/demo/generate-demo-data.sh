...
EOF
  ...
  while [ $i -le $num_points ]; do
    ...
    local hr_variation=$(( (RANDOM % 11) - 5 ))
...
period1_end=$(date -d "$TODAY - 64 days" +%Y-%m-%d)
response=$(mktemp)
status=$(curl -s -w "%{http_code}" -o "$response" -X POST "$API_URL/training/period" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"Base Building\",\"start\":\"$period1_start\",\"end\":\"$period1_end\",\"description\":\"Building aerobic base with easy miles\",\"sports\":null}")
...
period2_end=$(date -d "$TODAY - 36 days" +%Y-%m-%d)
response=$(mktemp)
status=$(curl -s -w "%{http_code}" -o "$response" -X POST "$API_URL/training/period" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"Build Phase\",\"start\":\"$period2_start\",\"end\":\"$period2_end\",\"description\":\"Introducing tempo runs and longer rides\",\"sports\":null}")
...
period3_end=$(date -d "$TODAY - 8 days" +%Y-%m-%d)
response=$(mktemp)
status=$(curl -s -w "%{http_code}" -o "$response" -X POST "$API_URL/training/period" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"Peak Training\",\"start\":\"$period3_start\",\"end\":\"$period3_end\",\"description\":\"Maximum training load with quality workouts\",\"sports\":null}")
...
status=$(curl -s -w "%{http_code}" -o "$response" -X POST "$API_URL/training/period" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"Race Week\",\"start\":\"$period4_start\",\"end\":\"$period4_end\",\"description\":\"Taper week with reduced volume\",\"sports\":null}")
...
metric_start=$(date -d "$TODAY - 120 days" +%Y-%m-%d)
response=$(mktemp)
# echo $TODAY
